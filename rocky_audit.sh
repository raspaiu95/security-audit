...
# [U-02] 패스워드 복잡도 설정: 영문, 숫자, 특수문자 조합 등 복잡도 설정이 pwquality.conf에 되어있는지 확인
# (Rocky 9은 Ubuntu와 달리 /etc/security/pwquality.conf를 주 설정 파일로 사용합니다.)
check_u02() {
    local f="/etc/security/pwquality.conf"
    if [ -f "$f" ]; then
        # 주석(#)과 빈 줄을 제외한 실제 설정된 옵션들을 한 줄로 추출
        local settings=$(grep -vE "^#|^$" "$f" | xargs)
        if [ -n "$settings" ]; then
            report "U-02" "패스워드 복잡도 설정" "양호" "설정됨: $settings"
        else
            report "U-02" "패스워드 복잡도 설정" "취약" "pwquality.conf에 설정된 정책이 없음"
        fi
    else
        report "U-02" "패스워드 복잡도 설정" "취약" "pwquality.conf 파일이 존재하지 않음"
    fi
}

# [U-03] 계정 잠금 임계값 설정: 로그인 실패 시 계정 잠금 기능(pam_faillock)이 활성화되어 있는지 확인
# (Rocky 9은 authselect 명령어를 통해 시스템 인증 구조를 관리합니다.)
check_u03() {
    # authselect를 통해 faillock 기능이 활성화되어 있는지 확인
    local auth_check=$(authselect current 2>/dev/null | grep "with-faillock")
    if [ -n "$auth_check" ]; then
        report "U-03" "계정 잠금 임계값 설정" "양호" "faillock 모듈이 시스템에 활성화됨"
    else
        # 개별 PAM 설정 파일에 faillock이 명시되어 있는지 추가 확인
        if grep -q "pam_faillock.so" /etc/pam.d/password-auth; then
            report "U-03" "계정 잠금 임계값 설정" "양호" "PAM 설정 내 faillock 적용 확인"
        else
            report "U-03" "계정 잠금 임계값 설정" "취약" "계정 잠금 정책(faillock)이 설정되지 않음"
        fi
    fi
}
...
# [U-13] 비밀번호 알고리즘 점검: 패스워드 해시 알고리즘이 안전한(SHA512 또는 yescrypt) 방식인지 확인
# (Rocky 9은 기본적으로 yescrypt를 사용하며, authselect 설정이 우선됩니다.)
check_u13() {
    # authselect 설정 및 login.defs에서 알고리즘 추출 후 한 줄 정리
    local algo=$(authselect current 2>/dev/null | grep "with-sha512" || grep -i "^ENCRYPT_METHOD" /etc/login.defs | awk '{print $2}')
    algo=$(echo $algo | xargs)
    
    if [[ "$algo" == *"SHA512"* || "$algo" == "yescrypt" ]]; then
        report "U-13" "비밀번호 알고리즘" "양호" "안전한 알고리즘($algo) 사용 중"
    else
        report "U-13" "비밀번호 알고리즘" "취약" "설정값: ${algo:-미설정} (SHA512 또는 yescrypt 권고)"
    fi
}
...
# [U-29] 네트워크 설정 파일 권한 설정: 네트워크 인터페이스 설정 파일의 무단 수정을 방지하기 위해 권한 점검
# (Rocky 9은 NetworkManager의 system-connections 파일을 주요 점검 대상으로 합니다.)
check_u29() {
    local nm_dir="/etc/NetworkManager/system-connections"
    if [ -d "$nm_dir" ]; then
        # 그룹/타인에게 쓰기 권한(022)이 있는 파일이 있는지 점검
        local insecure=$(find "$nm_dir" -type f -perm /022 2>/dev/null)
        if [ -z "$insecure" ]; then
            report "U-29" "네트워크 설정파일 권한" "양호" "모든 연결 파일의 권한이 제한됨"
        else
            report "U-29" "네트워크 설정파일 권한" "취약" "권한 과다 파일 발견: $(echo $insecure | xargs)"
        fi
    else
        report "U-29" "네트워크 설정파일 권한" "양호" "NetworkManager 설정 디렉터리 없음"
    fi
}

# [U-30] UMASK 설정 관리: 파일 생성 시 기본 권한을 제어하는 UMASK 값이 보안 기준(022)에 적합한지 점검
# (Rocky 9은 /etc/bashrc와 /etc/profile이 주요 설정 지점입니다.)
check_u30() {
    local global_umask=$(grep -i "umask" /etc/bashrc | grep -v "#" | awk '{print $2}' | head -n 1)
    if [ "$global_umask" == "022" ] || [ "$global_umask" == "0022" ]; then
        report "U-30" "UMASK 설정 관리" "양호" "시스템 기본 UMASK가 022로 적절함"
    else
        report "U-30" "UMASK 설정 관리" "취약" "설정값: ${global_umask:-미설정} (022 권고)"
    fi
}
...
# [U-33] DNS 보안 버전 패치: DNS 서비스(BIND)의 취약점이 해결된 버전 사용 여부 확인
# (Ubuntu의 apt 대신 Rocky의 rpm 패키지 매니저를 사용하여 버전을 확인합니다.)
check_u33() {
    if rpm -q bind &>/dev/null; then
        local dns_ver=$(rpm -q bind --qf "%{VERSION}")
        report "U-33" "DNS 보안 패치 점검" "수동확인" "BIND 설치됨 (버전: $dns_ver). 최신 보안 패치 여부 확인 필요"
    else
        report "U-33" "DNS 보안 패치 점검" "양호" "DNS 서비스(BIND)가 설치되지 않음"
    fi
}

# [U-34] Finger 서비스 비활성화: 사용자 정보를 노출하는 Finger 서비스의 활성화 여부 점검
# (Rocky 9은 xinetd 대신 systemd를 주로 사용하며, 서비스명이 finger-server일 수 있습니다.)
check_u34() {
    if systemctl is-active --quiet finger-server 2>/dev/null || [ -f /etc/xinetd.d/finger ]; then
        report "U-34" "Finger 서비스 비활성화" "취약" "Finger 서비스가 활성화되어 있음"
    else
        report "U-34" "Finger 서비스 비활성화" "양호" "Finger 서비스가 비활성화되어 있음"
    fi
}

# [U-35] Anonymous FTP 비활성화: 익명 FTP 접속 허용 여부를 점검하여 외부 유출 방지
# (Rocky 9의 vsftpd 설정 파일 기본 경로는 /etc/vsftpd/vsftpd.conf 입니다.)
check_u35() {
    local f_conf="/etc/vsftpd/vsftpd.conf"
    if [ -f "$f_conf" ]; then
        local anon_check=$(grep -i "^anonymous_enable" "$f_conf" | grep -i "YES")
        if [ -z "$anon_check" ]; then
            report "U-35" "Anonymous FTP 비활성화" "양호" "익명 접속이 차단되어 있음"
        else
            report "U-35" "Anonymous FTP 비활성화" "취약" "익명 접속이 허용되어 있음"
        fi
    else
        report "U-35" "Anonymous FTP 비활성화" "양호" "FTP 서비스(vsftpd)가 설치되지 않음"
    fi
}
...
# [U-38] DoS 공격에 취약한 서비스 비활성화: echo, discard 등 불필요한 서비스 점검
# (Rocky 9은 해당 서비스들이 패키지 형태로 별도 관리되거나 시스템 유닛으로 존재합니다.)
check_u38() {
    local dos_svc=("echo-stream" "echo-dgram" "discard-stream" "discard-dgram" "daytime-stream" "daytime-dgram" "chargen-stream" "chargen-dgram")
    local found=""
    for svc in "${dos_svc[@]}"; do
        if systemctl is-active --quiet "$svc" 2>/dev/null; then found+="$svc "; fi
    done
    if [ -z "$found" ]; then
        report "U-38" "DoS 취약 서비스 비활성화" "양호" "불필요한 DoS 유발 서비스가 비활성 상태임"
    else
        report "U-38" "DoS 취약 서비스 비활성화" "취약" "활성 중인 서비스 발견: $found"
    fi
}
...
# [U-46] 메일 서비스 버전 점검: Sendmail 또는 Postfix의 최신 보안 패치 여부 확인
# (Rocky 9은 기본 메일 서버로 Postfix를 주로 사용하므로 rpm을 통한 버전 확인이 필요합니다.)
check_u46() {
    if rpm -q postfix &>/dev/null; then
        local mail_ver=$(rpm -q postfix --qf "%{VERSION}")
        report "U-46" "메일 서비스 버전 점검" "수동확인" "Postfix 설치됨 (버전: $mail_ver). 최신 패치 여부 확인 필요"
    elif rpm -q sendmail &>/dev/null; then
        local mail_ver=$(rpm -q sendmail --qf "%{VERSION}")
        report "U-46" "메일 서비스 버전 점검" "수동확인" "Sendmail 설치됨 (버전: $mail_ver). 최신 패치 여부 확인 필요"
    else
        report "U-46" "메일 서비스 버전 점검" "양호" "메일 서버 소프트웨어가 설치되지 않음"
    fi
}
...
# [U-49] 불필요한 웹 서비스 제거: 시스템에 불필요한 웹 서버(Apache 등) 구동 여부 점검
# (Ubuntu의 apache2와 달리 Rocky 9의 서비스명은 httpd 입니다.)
check_u49() {
    if systemctl is-active --quiet httpd 2>/dev/null || systemctl is-active --quiet nginx 2>/dev/null; then
        report "U-49" "불필요한 웹 서비스 제거" "수동확인" "웹 서비스(httpd/nginx) 구동 중. 필요성 검토 필요"
    else
        report "U-49" "불필요한 웹 서비스 제거" "양호" "구동 중인 웹 서비스가 없음"
    fi
}

# [U-50] Apache 디렉토리 리스팅 제거: 브라우저에서 파일 목록이 노출되는 설정 점검
# (Rocky 9의 Apache 설정 파일 경로는 /etc/httpd/conf/httpd.conf 입니다.)
check_u50() {
    local h_conf="/etc/httpd/conf/httpd.conf"
    if [ -f "$h_conf" ]; then
        # Options 내에 Indexes 설정이 포함되어 있고 주석 처리되지 않았는지 확인
        if grep -r "Options" /etc/httpd/conf* | grep "Indexes" | grep -v "#" > /dev/null; then
            report "U-50" "웹 디렉토리 리스팅 제거" "취약" "Indexes 옵션이 활성화되어 파일 목록 노출 위험이 있음"
        else
            report "U-50" "웹 디렉토리 리스팅 제거" "양호" "디렉토리 리스팅 설정이 적절히 차단됨"
        fi
    else
        report "U-50" "웹 디렉토리 리스팅 제거" "양호" "Apache(httpd) 서비스가 설치되지 않음"
    fi
}
...
# [U-55] Apache 웹 프로세스 권한 제한: 웹 서비스가 root 권한으로 구동되는지 점검
# (Rocky 9의 httpd 설정 파일 내 User/Group 설정을 확인합니다.)
check_u55() {
    local h_conf="/etc/httpd/conf/httpd.conf"
    if [ -f "$h_conf" ]; then
        local user=$(grep "^User" "$h_conf" | awk '{print $2}')
        if [[ "$user" == "root" ]]; then
            report "U-55" "웹 프로세스 권한 제한" "취약" "프로세스가 root 권한으로 설정됨"
        else
            report "U-55" "웹 프로세스 권한 제한" "양호" "설정된 사용자: ${user:-apache}"
        fi
    else
        report "U-55" "웹 프로세스 권한 제한" "양호" "Apache(httpd) 미설치"
    fi
}
...
# [U-58] Apache 웹 설정 파일 권한 제어: 설정 파일의 무단 변조 방지 점검
# (Ubuntu의 /etc/apache2 대신 Rocky의 /etc/httpd 경로를 점검합니다.)
check_u58() {
    local h_conf="/etc/httpd/conf/httpd.conf"
    if [ -f "$h_conf" ]; then
        local perm=$(stat -c %a "$h_conf")
        [[ "$perm" -le 644 ]] && report "U-58" "웹 설정 파일 권한" "양호" "권한: $perm" \
        || report "U-58" "웹 설정 파일 권한" "취약" "권한 과다: $perm"
    else
        report "U-58" "웹 설정 파일 권한" "양호" "Apache 미설치"
    fi
}
...
# [U-60] 최신 보안 패치 적용: 시스템 보안 업데이트 상태 점검
# (Ubuntu의 apt 대신 Rocky의 dnf 보안 업데이트 확인 기능을 사용합니다.)
check_u60() {
    # dnf의 security 플래그를 사용하여 미적용된 보안 업데이트 개수 확인
    local updates=$(dnf check-update --security -q | grep -v "^$" | wc -l)
    if [ "$updates" -gt 0 ]; then
        report "U-60" "최신 보안 패치 적용" "수동확인" "$updates 개의 보안 업데이트가 존재함"
    else
        report "U-60" "최신 보안 패치 적용" "양호" "보안 패치가 모두 적용된 상태임"
    fi
}
...
# [U-62] ftpusers 파일 설정: root 등 관리자 계정의 접속 차단 여부 점검
# (Rocky 9의 vsftpd는 /etc/vsftpd/ftpusers 경로를 사용합니다.)
check_u62() {
    local ftp_users="/etc/vsftpd/ftpusers"
    if [ -f "$ftp_users" ]; then
        if grep -q "^root" "$ftp_users"; then
            report "U-62" "ftpusers 파일 설정" "양호" "root 계정 접속이 차단됨"
        else
            report "U-62" "ftpusers 파일 설정" "취약" "root 계정 접속이 허용되어 있음"
        fi
    else
        report "U-62" "ftpusers 파일 설정" "양호" "FTP 서비스 미사용 또는 파일 없음"
    fi
}
...
# [U-66] 로그 파일 권한 설정: 로그 변조 방지를 위한 권한 점검
# (Ubuntu의 auth.log/syslog 대신 Rocky의 secure/messages 경로를 점검합니다.)
check_u66() {
    local log_files=("/var/log/messages" "/var/log/secure" "/var/log/maillog")
    local insecure=""
    for log in "${log_files[@]}"; do
        if [ -f "$log" ]; then
            local perm=$(stat -c %a "$log")
            # 640(rw-r-----) 이하 권장
            if [ "$perm" -gt 640 ]; then
                insecure+="$log($perm) "
            fi
    ...
    if [ -z "$insecure" ]; then
        report "U-66" "로그 파일 권한 설정" "양호" "주요 로그 파일 권한 적절"
    else
        report "U-66" "로그 파일 권한 설정" "취약" "권한 과다: $(echo $insecure | xargs)"
    fi
}

# [U-67] 접속 IP 및 포트 제한: 방화벽 설정 여부 점검
# (Ubuntu의 ufw 대신 Rocky 9의 firewalld 상태를 점검합니다.)
check_u67() {
    if systemctl is-active --quiet firewalld 2>/dev/null; then
        local zone=$(firewall-cmd --get-default-zone)
        report "U-67" "접속 IP 및 포트 제한" "양호" "Firewalld 활성 중 (Default Zone: $zone)"
    else
        report "U-67" "접속 IP 및 포트 제한" "취약" "Firewalld 방화벽이 비활성 상태임"
    fi
}
